...
function [a, Fs] = test_functions(nameIn, len, mode, convState, windowSize, additionalFiltering, percentCutoff, LP_dB_Reduction, HPCO)
    ...
    if (mode == 1)
        ...
        z = decon_function(z, Fs, convState);
        %additional filtering
        if(additionalFiltering == 2)
            z = lowPass(z, percentCutoff, LP_dB_Reduction);
        elseif (additionalFiltering == 3)
            z = lowestRemoval(z, percentCutoff, LP_dB_Reduction);
        end
    
    %gaussian
    elseif (mode == 2)
        disp("starting gaussian filtering")
        z = gaussian(y, windowSize, Fs, HPCO);
        %Homomorphic filtering
        z = decon_function(z, Fs, convState);
        %additional filtering
        if(additionalFiltering == 2)
...
%         xlabel('Samples');
%         ylabel('Amplitude');
%         legend({'Filtered Signal','Original Signal','Noise'},'Location', 'northeast');
